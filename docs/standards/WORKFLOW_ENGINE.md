...
## 5. Incident Cancel Approve Verification Matrix (บังคับใช้ทุกหน้าจอ)

สำหรับการกด `Cancel Approve` ของ Incident ให้ใช้ policy กลางเดียวกันทั้ง UI และ Server Action:

1. `admin`:
- ต้องยืนยันด้วย `PIN ของ admin` เท่านั้น

2. `reporter account` (เจ้าของผู้แจ้ง):
- ต้องยืนยันด้วย `PIN ของ reporter` เท่านั้น

3. `it_staff` หรือ `assignee`:
- ถ้าเป็นผู้แจ้งภายใน (มี account): ยืนยันด้วย `PIN หรือ OTP ของ reporter`
- ถ้าเป็นผู้แจ้งภายนอก (external reporter): ยืนยันด้วย `OTP ทางอีเมล` เท่านั้น

หมายเหตุ:
- ห้ามกระจาย if/else role เฉพาะหน้า ให้เรียก policy กลางเพียงจุดเดียว
- ฝั่ง server ต้อง verify ซ้ำตาม policy เดียวกันเสมอ เพื่อกันการยิง API โดยตรง
...
### 4.3 Admin Reject Override (Incident Pending Approval)
เพื่อรองรับกรณีบัญชีผู้อนุมัติใช้งานไม่ได้ ระบบ Incident ต้องรองรับสิทธิ์พิเศษดังนี้:
1. `admin` สามารถสั่ง `reject` เอกสาร Incident ที่สถานะ `Pending Approval` ได้ทุกใบ
2. สิทธิ์พิเศษนี้จำกัดเฉพาะ `reject` เท่านั้น และไม่เท่ากับสิทธิ์ `approve` แทนทุกขั้นตอน
3. ฝั่ง Server Action ต้องตรวจสอบ role `admin` ก่อนอนุญาตทุกครั้ง ห้ามพึ่งพาเงื่อนไข UI เพียงอย่างเดียว
4. เมื่อ `reject` สำเร็จ ให้บันทึก audit log มาตรฐาน `Rejected` พร้อมเหตุผล

### 4.4 Admin Cancel Verification (Incident)
เมื่อผู้ที่กดยกเลิกเอกสาร Incident เป็น `admin` ให้ใช้กติกาตรวจสอบตัวตนดังนี้:
