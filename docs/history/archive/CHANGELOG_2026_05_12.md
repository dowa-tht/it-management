## [2026-05-12 17:30] - Incident Accept/Dispatch Audit-Safe Implementation Plan
- **Implementation Plan**: จัดทำแผน `IMPLEMENTATION_PLAN_INCIDENT_ACCEPT_DISPATCH_AUDIT.md` สำหรับปรับ Incident Accept/Dispatch ให้รองรับหลัก Audit และ Segregation of Duties
- **Architecture Decision**: กำหนดให้ `it_staff` กด Accept เพื่อรับงานเป็นของตนเองโดยตรง ส่วน `administrator` ทำได้เฉพาะ Dispatch โดยต้องเลือก active `it_staff` ก่อนบันทึก
- **Audit Control**: ระบุเงื่อนไขห้าม `administrator` ถูกบันทึกเป็น `incidents.assigned_to_id` ผ่าน Accept/Dispatch flow และต้องแยก log `รับเรื่อง (Acknowledge)` กับ `มอบหมายงาน (Dispatch)`
- **Documentation Index**: เพิ่มลิงก์แผนงานใน `docs/INDEX.md` เพื่อให้ agent อื่นเข้าถึงและดำเนินการต่อได้

---

## [2026-05-12 16:55] - Incident Creator Self-Approval Fix
- **Bug Fix**: แก้ไข Bug "PIN Incorrect" เมื่อเจ้าของเอกสารพยายามอนุมัติสเต็ป Reporter ผ่านช่องทาง Remote Approve
- **Logic Update**: ปรับปรุง `canApprove` ในหน้า Incident Detail ให้ระบุว่า Creator (คนสร้างเอกสาร) มีสิทธิ์อนุมัติสเต็ป `role_required = reporter` ได้โดยตรง (Direct Approval)
- **Security Compliance**: ปิดช่องโหว่ทาง Logic ที่บังคับให้คนสร้างเอกสารต้องกรอก PIN ของตัวเองผ่าน Remote Mode ทั้งที่เป็นเจ้าของเอกสารจริง
- **Audit Compliance**: บันทึกรายการลงใน `AUDIT_REPORT_LATEST.md` และยืนยันสถานะเป็น **PASSED**
