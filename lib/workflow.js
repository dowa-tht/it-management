...
// 🛡️ [Phase 1] Preparation for PIN Validation Cutover
...
export async function validateSignaturePin(userId, pin) {
  ...
  // 2. 🟢 FULL CUTOVER (Phase 3): Server Action PIN Verification
  // We use the centralized server action which safely verifies bcrypt hash
  // and handles failed attempts tracking internally.
  const res = await verifyEmployeePIN(userId, pin)
...
export async function getEligibleApprovers(targetType, triggerKey) {
  ...
  const { data: users } = await supabaseAdmin
    .from('user_profiles')
    .select('id, full_name, role, signature_pin')
