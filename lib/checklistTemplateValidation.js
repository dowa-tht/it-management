...
const procedurePlanLinkSchema = z.object({
...
const procedureConfigSchema = z.object({
  // Legacy: single plan_id (for backward compatibility)
  plan_id: z.string().trim().optional(),
  // New: multiple plan links
  plan_links: z.array(procedurePlanLinkSchema).default([]),
  enforce_sequence: z.boolean().default(true),
  require_all_steps: z.boolean().default(true),
}).refine((data) => {
  // Must have at least one plan (either legacy plan_id or new plan_links)
  const hasLegacyPlan = !!data.plan_id && data.plan_id.trim().length > 0
  const hasPlanLinks = Array.isArray(data.plan_links) && data.plan_links.length > 0
  return hasLegacyPlan || hasPlanLinks
}, {
  message: 'ต้องเลือกอย่างน้อย 1 Procedure Plan',
  path: ['plan_links'],
})
...
const DEFAULT_CONFIGS = {
  ...
  2: {
    plan_id: '',
    plan_links: [],  // Array of { plan_id, is_default, sort_order }
...
function sanitizeConfigForType(type, config) {
  ...
  switch (type) {
    ...
    case 2: {
      // Normalize plan_links array
      const rawPlanLinks = Array.isArray(source.plan_links) ? source.plan_links : []
      const planLinks = rawPlanLinks
        .filter(p => p && (p.plan_id || (p.id && typeof p.id === 'string')))
        .map((p, idx) => ({
          plan_id: String(p.plan_id || p.id || '').trim(),
          is_default: p.is_default === true || (idx === 0 && !rawPlanLinks.some(x => x.is_default)),
          sort_order: typeof p.sort_order === 'number' ? p.sort_order : idx,
        }))
        .filter(p => p.plan_id.length > 0)
...
export function buildTemplatePreview(template, procedurePlans = []) {
  ...
  switch (normalized.ui_template_type) {
    ...
    case 2: {
      // Support both legacy single plan and new multiple plans
      const planLinks = Array.isArray(config.plan_links) ? config.plan_links : []
      const legacyPlanId = config.plan_id

      let planNames = []

      if (planLinks.length > 0) {
        // New: multiple plan links
        planNames = planLinks.map(link => {
          const matchedPlan = procedurePlans.find(p => p.id === link.plan_id)
          const name = matchedPlan?.plan_name || link.plan_id
          return link.is_default ? `${name} (default)` : name
        })
      } else if (legacyPlanId) {
        // Legacy: single plan
        const matchedPlan = procedurePlans.find(p => p.id === legacyPlanId)
        planNames = [matchedPlan?.plan_name || legacyPlanId]
      }

      preview.lines = [
        `Procedure plans: ${planNames.length > 0 ? planNames.join(', ') : 'Not selected'}`,
