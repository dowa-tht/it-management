...
export async function peekNextNo(code, workingDate = new Date(), supabaseClient = null) {
  ...
  const basisNo = persistedDocNo || context.lastNoUsed
  const nextNum = basisNo && String(basisNo).startsWith(context.prefixPart)
    ? getSequenceFromDocNo(basisNo, context.hashCount) + 1
    : 1

  return {
    nextNo: `${context.prefixPart}${String(nextNum).padStart(context.hashCount, '0')}`,
    series: context.line || context.header,
    header: context.header,
    line: context.line,
    lineId: context.lineId,
