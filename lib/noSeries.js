 ...
 * @param {object} supabaseClient - Optional supabase client (use for server-side admin)
 */
export async function getNextNo(code, workingDate = new Date(), supabaseClient = null) {
  ...
  if (error || !line) {
    const { data: header } = await client.from('no_series').select('*').eq('code', code).single()
  ...
  const linkedForm = isLegacy ? headerData?.linked_form : line?.no_series?.linked_form;
  const mapping = NO_SERIES_DOC_MAPPING[linkedForm] || NO_SERIES_DOC_MAPPING[code] || NO_SERIES_DOC_MAPPING['ไม่ผูกกับเอกสาร'];

  const tableName = mapping?.tableName || null;
  const colName = mapping?.colName || null;

  // Auto-Max Single Query Optimization (Only if linked to a table)
  let maxRecord = null;
  ...
  // When not linked to a table, use database atomic updates (or RPC) for concurrency control if possible.
  // In Supabase REST API, an update with eq() and returning() can help, but it might not perfectly prevent race conditions
  // without an RPC. However, awaiting the update ensures the DB is updated before returning.

  if (!tableName || !colName) {
    // Await the update to ensure it's written sequentially for "ไม่ผูกกับเอกสาร"
    if (!isLegacy && lineId) {
      await client.from('no_series_lines').update({ last_no_used: nextNo }).eq('id', lineId)
    } else if (isLegacy && headerData) {
      await client.from('no_series').update({ last_no_used: nextNo }).eq('id', headerData.id)
    }
  } else {
    // Background update (fire and forget) to keep setup UI in sync when relying on maxRecord
    if (!isLegacy && lineId) {
      client.from('no_series_lines').update({ last_no_used: nextNo }).eq('id', lineId).then()
    } else if (isLegacy && headerData) {
      client.from('no_series').update({ last_no_used: nextNo }).eq('id', headerData.id).then()
    }
  }
 ...
 */
export async function updateLastNo(code, no, lineId = null, supabaseClient = null) {
  const client = supabaseClient || supabase
  if (lineId) {
    await client
      .from('no_series_lines')
