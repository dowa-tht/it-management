...
const procedureStepSchema = z.object({
  ...
  success_criteria: z.string().trim().default(''),
...
export function normalizeProcedurePlanSteps(rawSteps) {
  ...
  return source.map((step, index) => {
    ...
    return {
      ...
      responsible_person: String(safeStep.responsible_person ?? '').trim(),
      success_criteria: String(safeStep.success_criteria ?? '').trim(),
