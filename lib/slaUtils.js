...
export function calculateNetBusinessMinutes(start, end, settings, holidays = [], exclusions = []) {
  ...
  current.setSeconds(0, 0); // Normalize to minutes
  
  // Optimization: Pre-calculate working hours in minutes for faster comparison
  ...
  while (current <= endDate) {
    ...
    if (settings.work_days.includes(dayOfWeek) && !holidays.includes(dayStr)) {
      ...
      const effectiveStart = new Date(Math.max(current, dayStart, startDate));
      const effectiveEnd = new Date(Math.min(endDate, dayEnd));
