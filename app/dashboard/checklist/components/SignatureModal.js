...
export function SignatureModal({ isOpen, onConfirm, onCancel, approverName, loading, userEmail }) {
      ...
      <div style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 450, overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ padding: '16px 24px 24px', background: '#f8fafc', display: 'flex', gap: 12 }}>
          ...
          <button 
            ...
            style={{ flex: 2, padding: '12px', borderRadius: 12, border: 'none', background: pin.length === 6 ? '#1d4ed8' : '#93c5fd', color: '#fff', fontWeight: 700, cursor: pin.length === 6 ? 'pointer' : 'not-allowed', boxShadow: pin.length === 6 ? '0 10px 15px -3px rgba(29, 78, 216, 0.3)' : 'none' }}
          >
